...
async def _execute_search_step(
    ...
    has_electronic_access: bool | None = None,
    ...
    - Filter params (catalog_type, year_from, year_to, author, has_electronic_access):
      applied to all retrieval calls to narrow down results.
    """
...
async def _stream_step_thinking(
    ...
    # Collect all chunks first to avoid cross-task cancel scope issues
    chunks = []
    ctx = dspy.context(lm=cheap_lm) if cheap_lm else contextlib.nullcontext()
    try:
        with ctx:
        ...
        logger.warning(f"[STREAM] Step thinking error: {e}")
...
async def stream_dspy_response(
    ...
    title_from_question_generator: Any = None,
    save_title_callback: Any = None,
    catalog_type: str | None = None,
    year_from: int | None = None,
    year_to: int | None = None,
    author: str | None = None,
    ...
    Args:
        ...
        generate_title: DEPRECATED - async callable(question, answer) -> str. Kept for backwards compat.
        title_from_question_generator: DSPy module to generate title from question only.
                       When provided, title is generated EARLY (parallel with intent classification),
                       eliminating blocking after 'done' event.
        save_title_callback: async callable(title: str) — called when title is generated early.
                      Use this to persist title to DB immediately (non-blocking).
    ...
    SSE event contract (in emission order):
      ...
      title         { content }            ← now non-blocking!
      error         { content }
    """
    ...
    try:
        ...
        intent_task = None
        query_task = None
        title_task = None  # Early title generation (parallel)
        ...
        # Generate title from question EARLY (parallel with intent classification)
        # This eliminates blocking after 'done' event
        if title_from_question_generator:
            title_task = asyncio.create_task(
                _run_dspy_sync(title_from_question_generator, cheap_lm=cheap_lm, question=question)
            )

        is_research = True
        pre_generated_query: str | None = None
        pre_generated_title: str | None = None  # Will store title once ready
        if intent_task:
            try:
        ...
        if not is_research:
            ...
            async for value in streaming_program(
                ...
                if isinstance(value, dspy.streaming.StreamResponse) and value.chunk:
                    ...
                    # Emit title (use pre-generated if available, else fallback)
                    if title_task:
                        try:
                            title_result = await title_task
                            title = getattr(title_result, 'title', None)
                            if title:
                                yield format_sse({"type": "title", "content": title})
                                # Save to DB (fire-and-forget)
                                if save_title_callback:
                                    asyncio.create_task(save_title_callback(title))
                        except Exception as _te:
                            logger.warning("[STREAM] Pre-title collection error: %s", _te)
                    elif generate_title:
                        try:
        ...
        if title_task:
            try:
                title_result = await title_task
                pre_generated_title = getattr(title_result, 'title', None)
                if pre_generated_title:
                    logger.info("[STREAM] Pre-generated title: '%s'", pre_generated_title)
                    # Save title to DB immediately (non-blocking via fire-and-forget)
                    if save_title_callback:
                        asyncio.create_task(save_title_callback(pre_generated_title))
            except asyncio.CancelledError:
                pre_generated_title = None
            except Exception as _te:
                logger.warning("[STREAM] Pre-title generation failed (%s)", _te)
                pre_generated_title = None
        yield format_sse(
        ...
        # Emit title (non-blocking if pre-generated, else fallback to old method)
        if pre_generated_title:
            # Title was already generated in parallel - emit immediately!
            yield format_sse({"type": "title", "content": pre_generated_title})
            logger.info("[STREAM] Title emitted immediately (pre-generated)")
        elif generate_title:
            # Fallback: old method (blocks until title is generated)
            try:
